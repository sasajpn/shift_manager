...
RSpec.describe Api::V1::Owners::ShiftRegistrationsController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    context 'ログインしている場合' do
      context 'シフト登録がログイン済みのオーナーのものである場合' do
        it 'シフト登録が作成される' do
          expect{
            post :create, params: { member_id: member.id, shift_registration: { registered_date: Date.today, start_time: '10:00', end_time: '12:00' } }
      ...
      context 'シフト登録がログイン済みのオーナーのチームのものでない場合' do
        it 'シフト登録が作成されない' do
          expect{
            post :create, params: { member_id: other_member.id, shift_registration: { registered_date: Date.today, start_time: '10:00', end_time: '12:00' } }
          }.to change(Shift::Registration, :count).by(0)
        end
        it '404が返ってくる' do
          post :create, params: { member_id: other_member.id }
          expect(response).not_to be_success
          expect(response.status).to eq 404
        end
      end
  ...
  describe 'PATCH #update' do
    ...
    context 'ログインしている場合' do
      context 'シフト登録がログイン済みのオーナーのものである場合' do
        it 'シフト登録が更新される' do
          patch :update, params: { id: shift_registration.id, shift_registration: { start_time: '10:00' } }
      ...
      context 'シフト登録がログイン済みのオーナーのチームのものでない場合' do
        it 'シフト登録が更新されない' do
          patch :update, params: { id: other_shift_registration.id, shift_registration: { start_time: '10:00' } }
          shift_registration.reload
