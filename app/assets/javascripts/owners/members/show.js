    ...
    eventClick: function(event, jsEvent, view) {
      if (view.type === 'listDay') {
        if (event.shift_submission_id) {
          window.location.href = '/owners/shift_submissions/' + event.shift_submission_id + '?referrer=members_shift_calendar'
        } else if (event.shift_registration_id) {
          window.location.href = '/owners/shift_registrations/' + event.shift_registration_id + '/edit?referrer=members_shift_calendar'
        }
      }
    }
  })
